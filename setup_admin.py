...
def create_dept_manager(email, name, password, category_name):
    ...
    with app.app_context():
        ...
        if user:
            ...
            db.session.add(user)
            print(f"✅ Created New Manager: {name}")
        ...
        if not category:
            print(f"❌ Error: Category '{category_name}' does not exist! (Check spelling)")
            return
        ...
        if not link:
            ...
            db.session.commit()
            print(f"🔗 Linked {name} to {category_name} Department.")
        else:
            print(f"ℹ️ {name} is already linked to {category_name}.")
